...
public class ViewUtil {
    ...
    public static void setPicture(Uri uri, int defaultRes, ImageView iv, ImageLoadingListener listener) {
        if (uri != null) {
            ImageAware imageAware = new ImageViewAware(iv, false);
            if (listener != null) {
                ImageLoader.getInstance().displayImage("file://" + uri.toString(), imageAware, DisplayConfig.getDefaultOptions(false, defaultRes), listener);
            } else {
                ImageLoader.getInstance().displayImage("file://" + uri.toString(), imageAware, DisplayConfig.getDefaultOptions(false, defaultRes));
            }
        } else {
            iv.setImageResource(defaultRes);
        }
